...
class OCRModule(pl.LightningModule):
    ...
    def test_step(
        ...
        max_len = max(preds_encoded.size(1), targets.size(1))
        if preds_encoded.size(1) < max_len:
            preds_encoded = F.pad(preds_encoded, (0, max_len - preds_encoded.size(1)), "constant", 0)
        if targets.size(1) < max_len:
            targets = F.pad(targets, (0, max_len - targets.size(1)), "constant", 0)

        # compute accuracy
        #self.test_accuracy.update(preds_encoded, targets)
        #self.log("test_acc", self.test_accuracy, prog_bar=True)  # type: ignore#################################################預測台灣車牌會抱錯
    ...
    def test_epoch_end(self, outs: list[dict[str, Any]]) -> None:  # type: ignore
        ...
        #self.logger.experiment.add_figure("test_confusion_matrix", fig)  # type: ignore   ####################################################################################預測台灣車牌會抱錯
